...
struct DisplayControllerTests {
    ...
    // Brightness write fails after mirroring: fail loudly and do NOT mark the display as successfully dimmed.
    @Test func failsLoudlyWhenBrightnessWriteFailsOnEngage() {
        let backend = MockDisplayBackend(
            displays: [.builtin(internalID), .external(externalID)],
            brightness: [internalID: 0.8],
            autoBrightness: [internalID: true],
            hasALC: [internalID: true])
        backend.brightnessSucceeds = false
        let state = tempState()
        var failures: [String] = []
        let c = DisplayController(backend: backend, state: state)
        c.notifyFailure = { failures.append($0) }
        ...
        #expect(state.state.lastError != nil)
        #expect(state.state.engaged == false)
        #expect(backend.mirrorMap[internalID] == externalID)   // mirror may have happened, but dim did not
        #expect(approx(backend.brightnessMap[internalID], 0.8))
    }
    ...
    // If that re-mirror heal fails, don't continue by dimming an extended, unmirrored built-in.
    @Test func mirrorMasterHealFailureDoesNotDimUnmirroredBuiltin() {
        let ext1: CGDirectDisplayID = 2
        let ext2: CGDirectDisplayID = 3
        let backend = MockDisplayBackend(
            displays: [.builtin(internalID), .external(ext1), .external(ext2)],
        ...
        let c = DisplayController(backend: backend, state: state)
        c.evaluate()
        let brightnessWritesAfterEngage = backend.brightnessSets.count

        backend.displays = [.builtin(internalID), .external(ext2)]
        backend.mirrorMap[internalID] = nil
        backend.brightnessMap[internalID] = 0.06
        backend.mirrorSucceeds = false
        c.evaluate()

        #expect(state.state.lastError != nil)
        #expect(backend.brightnessSets.count == brightnessWritesAfterEngage)
        #expect(approx(backend.brightnessMap[internalID], 0.06))
    }
    ...
    // Restore is the safety path; if the brightness write fails, preserve an explicit error instead of
    // silently claiming everything is fine.
    @Test func restoreFailureIsReported() {
        let backend = MockDisplayBackend(
            displays: [.builtin(internalID)],
            brightness: [internalID: 0],
            autoBrightness: [internalID: false],
        ...
        backend.brightnessSucceeds = false
        let state = tempState()
        state.update { $0.engaged = true; $0.priorBrightness = 0.7; $0.priorAutoBrightness = false }
        DisplayController(backend: backend, state: state).evaluate()
