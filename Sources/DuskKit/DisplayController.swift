...
public final class DisplayController {
    private struct DarkAssertion {
        var autoBrightnessHeld: Bool
        var brightnessHeld: Bool
        var succeeded: Bool { autoBrightnessHeld && brightnessHeld }
    }
    ...
    private func ensureDimmed(_ internalID: CGDirectDisplayID, externals: [CGDirectDisplayID]) {
        ...
        if state.state.engaged {
            ...
            if stillMirrored {
                ...
                reportIfDarkAssertionFailed(assertDark(internalID))
                return
            }
            ...
            if mirrorTargetUnplugged {
                ...
                guard backend.setMirror(of: internalID, to: newTarget) else {
                    fail("Failed to set mirroring; not applied")
                    return
                }
    ...
    /// Tracks brightness and auto-brightness separately: the former decides whether we actually dimmed, while
    /// the latter means "0 may not hold" and should be surfaced without pretending the brightness write failed.
    @discardableResult
    private func assertDark(_ internalID: CGDirectDisplayID) -> DarkAssertion {
        ...
        if (backend.brightness(of: internalID) ?? 1) > epsilon {
            brightnessHeld = backend.setBrightness(0, of: internalID)
        }
        return DarkAssertion(autoBrightnessHeld: autoBrightnessHeld, brightnessHeld: brightnessHeld)
    }

    private func reportIfDarkAssertionFailed(_ dark: DarkAssertion) {
        if !dark.brightnessHeld {
            fail("Could not set the built-in brightness to 0; it may remain visible")
        } else if !dark.autoBrightnessHeld {
            fail("Could not disable the built-in's auto-brightness (private symbol may be gone); it may re-light")
        }
    }
    ...
    private func yieldToUser(_ internalID: CGDirectDisplayID, switchedToExtended: Bool) {
        if state.state.priorAutoBrightness, backend.hasAutoBrightness(of: internalID) {
            if !backend.setAutoBrightness(true, of: internalID) {
                fail("Could not restore the built-in's auto-brightness")
            }
        }
        ...
        if (backend.brightness(of: internalID) ?? 0) < reclaimThreshold {
            ...
            if !backend.setBrightness(restoreTo, of: internalID) {
                fail("Could not restore the built-in brightness while handing it back")
            }
        }
    ...
    private func ensureRestored(_ internalID: CGDirectDisplayID) {
        if state.state.engaged {
            ...
            if !performRestore(internalID, force: false) { fail("Could not fully restore the built-in display") }
            ...
            if performRestore(internalID, force: true) {
                log?("Built-in was the only screen and dark; forced restore")
            } else {
                fail("Could not force-restore the only built-in display")
            }
        }
    ...
    @discardableResult
    private func performRestore(_ internalID: CGDirectDisplayID, force: Bool) -> Bool {
        var ok = true
        ...
        if force || current < darkThreshold {
            ...
            ok = backend.setBrightness(target, of: internalID) && ok
        }
        // Auto-brightness was originally on -> turn it back on and let it take over subsequent tweaks.
        if state.state.priorAutoBrightness, backend.hasAutoBrightness(of: internalID) {
            ok = backend.setAutoBrightness(true, of: internalID) && ok
        }
    ...
    public func restoreUnconditionally() {
        ...
        if !performRestore(builtin.id, force: true) {
            fail("Could not force-restore the built-in display")
        }
        restoreDeadline = nil
    }
