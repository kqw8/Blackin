...
public enum LaunchAgent {
    ...
    public static func isLoginEnabled() -> Bool {
        ...
        if let isDisabled = disabledState(for: label, in: output) { return !isDisabled }
        // Not in the disabled list -> launchd enables it by default.
        return true
    }

    static func disabledState(for label: String, in printDisabledOutput: String) -> Bool? {
        for line in printDisabledOutput.split(separator: "\n") where line.contains("\"\(label)\"") {
            let lower = line.lowercased()
            // macOS has emitted both boolean and word forms:
            //   "com.kqw8.dusk" => true       (disabled)
            //   "com.kqw8.dusk" => disabled   (disabled)
