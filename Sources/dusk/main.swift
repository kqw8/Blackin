...
func cmdInstall() {
    ...
    // Clear any stale disabled override first: on current macOS, bootstrapping a disabled
    // label can fail with rc=5. Disable again after the service is running to keep install's
    // "resident now, no start-at-login" contract.
    _ = LaunchAgent.enableLogin()
    guard LaunchAgent.bootstrap(), LaunchAgent.kickstart(), LaunchAgent.isBootstrapped() else {
    ...
    guard LaunchAgent.disableLogin() else {
        printErr("❌ Install failed: background service is running, but start-at-login could not be disabled.")
        exit(1)
    }
...
func cmdUninstall() {
    ...
    guard LaunchAgent.bootout() else {
        printErr("❌ Uninstall failed: could not unload the background service; not removing files while it may still be running.")
        exit(1)
    }
    _ = LaunchAgent.enableLogin()    // clear the launchctl disable override that install wrote
    guard LaunchAgent.removePlist() else {
        printErr("❌ Uninstall failed: could not remove the LaunchAgent plist.")
        exit(1)
    }
...
func cmdStart() {
    let loginWasEnabled = LaunchAgent.isLoginEnabled()
    ...
    _ = LaunchAgent.enableLogin()
    guard LaunchAgent.bootstrap(), LaunchAgent.kickstart(), LaunchAgent.isBootstrapped() else {
        if !loginWasEnabled { _ = LaunchAgent.disableLogin() }
        printErr("❌ Start failed: the background service did not load.")
        exit(1)
    }
    if !loginWasEnabled, !LaunchAgent.disableLogin() {
        printErr("❌ Start failed: service is running, but start-at-login could not be restored to off.")
        exit(1)
    }
    ...
    case "on":
        if !LaunchAgent.plistExists() {
            do {
                ...
                printErr("❌ Could not write LaunchAgent plist: \(error)")
                exit(1)
            }
        }
        guard LaunchAgent.enableLogin() else {
            printErr("❌ Could not enable start-at-login.")
            exit(1)
        }
        guard LaunchAgent.bootstrap() else {
            _ = LaunchAgent.disableLogin()
            printErr("❌ Start-at-login was not enabled because the background service could not load.")
            exit(1)
        }
        print("✅ Start-at-login enabled.")
    case "off":
        guard LaunchAgent.disableLogin() else {
            printErr("❌ Could not disable start-at-login.")
            exit(1)
        }
